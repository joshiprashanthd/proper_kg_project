...
parser.add_argument('--type', type=str, required=True,
                    choices=['mcqa', 'yes_no', 'long_form', 'classification_single', 'classification_multiple', 'long'],
...
if dataset_type == 'mcqa':
    for index, row in df.iterrows():
        ...
        try:
            answer, store = ask_llama_mcqa(question, opa, opb, opc, opd)
        except Exception as e:
            answer, store = 'error', 'error'
            print(f"Error processing question ID {id}: {e}")
        print(f"Progress: {counter}/{len}")
        print(answer)
        results.append({'id': id, 'source': f'{dataset_name}', 'question': question, 'answer': answer, 'store': store})
        counter+=1
...
results_df.to_csv(f'/home/sracha/proper_kg_project/scripts/qa_evaluation/results/{dataset_name}_llama_results.csv', index=False)
