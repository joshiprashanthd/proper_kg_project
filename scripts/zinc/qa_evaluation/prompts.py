...
def ask_llama_mcqa(question, opa, opb, opc, opd):
    ...
    Your task is to select the best answer and respond ONLY in the structure shown below.
...
def ask_llama_mcqa_cot(question, opa, opb, opc, opd):
    # Chain-of-Thought (CoT) prompt for MCQA
    prompt = f"""
    You are a mental health expert AI. You will be given a multiple choice question with four options.
    Your task is to reason through each option step by step, explain your reasoning for each, and then select the best answer.
    Respond ONLY in the structure shown below.

    Respond in this structure:
    "reasoning": "<step-by-step reasoning for each option and final answer selection>"
    "answer": "<selected_option_letter>"

    Example output:
    "reasoning": "Option A: ... Option B: ... Option C: ... Option D: ... After considering all options, the best answer is B because ..."
    "answer": "B"
    ...
    # Extract reasoning and answer from the response using regex
    reasoning_match = re.search(r'"reasoning":\s*"(.*?)"\s*"answer":', store, re.DOTALL)
    answer_match = re.search(r'"answer":\s*"(.*?)"', store)
    ...
    if reasoning_match and answer_match:
        ...
        raise Exception(f"Failed to extract reasoning and answer from response: {store}")


def mhqa_long(question):
    # Create a simple instruction-style prompt
    prompt = f"""
    You are a mental health expert. You are given a question, your task is to answer the question in a few words. Usually the answer would be a single or a few words.
    Output the complete answer only, and nothing else.

    Input Example 1:
    Question: How does elevated food worry during the COVID-19 pandemic impact mental health outcomes such as anxiety thoughts?
    Output answer: It leads to increased anxiety

    Input Example 2:
    Question: What brain region's activity correlates with pupil responses to threat cues during fear conditioning?
    Output answer: Amygdala
