...
class FaissRag:
    def __init__(self, top_k: int, vector_store_path: Path = Path("/home/sracha/proper_kg_project/src/components/faiss_rag/vector_store")):
    ...
    def run(self, query: str) -> list[RagDocument]:
        ...
        docs = [RagDocument(content=doc.page_content, score=-float(score), source="FaissRag", type="text") for (doc, score) in results]
