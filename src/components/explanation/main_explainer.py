...
class MainExplainer(BaseExplainer):
    def __init__(self):
        self.prompt_template = """You are a medical expert. Your goal is to generate an explanation *strictly* using *only* the provided reasoning paths such that the explanation helps in arriving to or understanding the correct answer to the given question.

The valid reasoning path format is: `(entity 1) -> relation 1 -> (entity 2) -> relation 2 -> .... -> (entity n)`
